...
const totalBask = document.querySelector('.basketTotal');
...
document.querySelector('.featuredItems').addEventListener('click', event => {
    if (!event.target.closest('.addToCrd')) {
        return;
    }
    ...
    addToCrd(id, name, price);
});

function addToCrd(id, name, price) {
    if (!(id in basket)) {
        basket[id] = {id: id, name: name, price: price, count: 0};
    }
    basket[id].count++;
    basketCountEl.textContent = getTotalBasketCount().toString();
    basketValueEl.textContent = getTotalBasketPrice().toFixed(2);
